package com.livestockhusbandry.entity.ai.shepherd;

import com.livestockhusbandry.block.ModBlocks;
import com.livestockhusbandry.block.entity.ShepherdRackBlockEntity;
...
public final class ShepherdShearingManager {
    ...
    private static boolean isValidTargetSheep(ServerLevel level, Sheep sheep) {
        ...
        BlockPos registeredTrough = SheepTroughReservations.getRegisteredTrough(
                level,
                sheep.getUUID()
        );

        if (registeredTrough == null) {
            return false;
        }

        TroughUtil.TroughGroup group = SheepTroughUtil.getRegisteredGroup(sheep);
